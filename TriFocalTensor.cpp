...
namespace sfm
{
		...
		Matrix3d &T)
	{
		...
		double cu = 0, cv = 0;
		double m = static_cast<double>(vPtsEnd.size() * 2);
		for (std::vector<Eigen::Vector4d>::iterator it = vPtsEnd.begin(); it != vPtsEnd.end(); ++it)
		{
			cu += (*it)[0];
			cu += (*it)[1];
			cv += (*it)[2];
		...
		for (std::vector<Eigen::Vector4d>::iterator it = vPtsEnd.begin(); it != vPtsEnd.end(); ++it)
		{
			Vector2d pt = Vector2d((*it)[0], (*it)[1]);
			sp += sqrt(pt[0] * pt[0] + pt[1] * pt[1]);
			pt = Vector2d((*it)[2], (*it)[3]);
			sp += sqrt(pt[0] * pt[0] + pt[1] * pt[1]);
		}
		if (fabs(sp)<1e-10)
			return false;
	...
	////////////////////////////////////////////////////////////
	bool TrifocalTensor::normalize2DLines(std::vector<std::pair<Eigen::Vector2d,
		Eigen::Vector2d> > &vPtsEndPair,
		Matrix3d &T)
	{
		// shift origins to centroids
		double cu = 0, cv = 0;
		double m = static_cast<double>(vPtsEndPair.size() * 2);
		for (std::vector<std::pair<Eigen::Vector2d, Eigen::Vector2d> >::iterator it = vPtsEndPair.begin(); it != vPtsEndPair.end(); ++it)
		{
			cu += it->first[0];
			cu += it->second[0];
			cv += it->first[1];
